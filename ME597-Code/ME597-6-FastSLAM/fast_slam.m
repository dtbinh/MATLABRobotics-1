...
% constant ang speed of 0.3 rad / s
u = ones(2, length(T));
u(2,:)=0.3*u(2,:);

% Feature Map
%   Get random Coordiantes between 
%   x : -5.5->6.5
%   y : -2 -> 10M = 20;
map = 12*rand(2,M);
map(1,:) = map(1,:)-5.5; 
...
for t=2:length(T)
    %% Simulation
    %    This for producing ground truth data to know the real traj
    ...
    % Take measurements of the "visible" markers
    y = [];
    for i = meas_ind
    ...
    % FAST SLAM is a combo of EKF and Particle Filter

    % For each particle
    %    slide 38 mapping 2
    %    Explanation: For each particle, go through all the measurements
    %    visible and calculate a weight (probability that it is the correct
    %    particle)
    for d=1:D
        % Select a motion disturbance
        ...
        for j=1:length(meas_ind)
            i = meas_ind(j);
            % If it is a new feature seen, initialize its particle once
            %   slide 39 mapping 2
            if (newfeature(i) == 1)
                % Feature initialization
