...
% Determines error size based on type of measurement
switch(meas)
    case 1
        Q = 0.02;
    case 2
...
[QE, Qe] = eig(Q);
% Sensor footprint
rmax = 10 ; % meters  % max scan distance
thmax = pi/4; % rads   % max scan angle
...
% Feature Map  --> pops in our features 
map = [ 5 5;  
         3  1;
         -4  5;
         -2  3;
...
for t=2:length(T)
    ...
    %% Particle filter estimation --> see PF notes
    % PF review:
    % 1. Make a bunch of samples
    % 2. Propagate samples forward
    % 3. resample
    % 4. Take measurement
    
    % updates proposal distribution for each sample
    for dd=1:D
        e = RE*sqrt(Re)*randn(n,1);
    ...
    if (nj > 0)
        for dd=1:D
            Qj = zeros(nj,nj);
            % nj = 4 if we see two targets, 
            % nj = 2 if we see 1 or less
            % targets
            hXp =[];
            for j=1:nj
                switch(meas)
            ...
            w(dd) = mvnpdf(y_fixed,hXp,Qj);   % multivariate normal probability density function
        end
